package com.example.csc475recipeapp.data.network

import com.example.csc475recipeapp.data.model.Meal
import com.example.csc475recipeapp.data.model.MealResponse
...
object MealApiClient {
    ...
    suspend fun getRandomRecipe(): List<Meal> {
        ...
        return response.meals.filterNotNull()
    }

    suspend fun getSearchedRecipe(query: String): List<Meal> {
        val url = "https://www.themealdb.com/api/json/v1/1/search.php?f=a$query"
        val response = apiClient.get(url).body() as MealResponse
        return response.meals.filterNotNull()
    }
